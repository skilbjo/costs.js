...
var Network = function(Qualification_Code, Network) {
	var visaRegex = /^VS /,
		mcRegex = /^MC /,
		mcRefundRegex =/^((Consumer )|(Commercial ))/,
		dsRegex = /^DS /,
		dsRefundRegex =/( DISCOVER)$/
	;

	if ( visaRegex.test(Qualification_Code) || (Network === 'VISA') || (Network === 'Clr VISA Int.') ) {
		return 'Visa';
	} else if ( mcRegex.test(Qualification_Code) || mcRefundRegex.test(Qualification_Code) || (Network === 'MasterCard') || (Network === 'Clr Mastercard Int') ) {
		return 'Mastercard';
	} else if ( dsRegex.test(Qualification_Code) || dsRefundRegex.test(Qualification_Code) || (Network === 'Discover') || (Network === 'Discover Diners') || (Network === 'JCB') ) {
		return 'Discover';
	}
...
var CardType_GlobalPayments = function(FPI_Code){
	// Send Angela @ GlobalPayments has the latest FPI debit codes. Last data received October 2015
	var Debit_FPI_Codes = 
		['01B','06B','08K','11L','59W','72W','A11','BB7','BI6','BU1','BU4','BW9','CC5','MG9','NM8','NN2','NN3','NN8','NW4',
		'J67','NM8','NN2','NN3','P72','01B','04D','72A','97G','P72'],
		result = 'Credit'
	;
